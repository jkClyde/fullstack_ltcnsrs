...
import {
  generateInitialSummaryCount,
  updateSummary,
  getLatestQuarter,
} from "./generateInitialSummaryCount";
...
const SummaryReport = () => {
  ...
  const chosenBarangay = useSelector(
    (state) => state.refresher.selectedBarangay
  );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const primaryChildResponse = await axios.get(
        ...
        const primaryChildData = primaryChildResponse.data;
        const childHealthData = childHealthInfoResponse.data;

        // Merging data
        mergedData = primaryChildData.map((child) => ({
          ...child,
          ...childHealthData.find(
            (healthInfo) => healthInfo.child === child.id
          ),
        }));

        filteredData =
          chosenBarangay === "All Barangays"
            ? mergedData
            : mergedData.filter((item) => item.barangay === chosenBarangay);
  ...
  useEffect(() => {
    if (data && !dataProcessed) {
      let newPopulation = 0; // Initialize a new population count variable

      data.forEach((patient) => {
        if (patient) {
          // Increment population count for each valid patient
          newPopulation += 1;

          const lfa_status = lengthForAgeStatus(
            patient.birthdate,
            patient.height,
            patient.gender
          );
          const wfa_status = weightForAgeStatus(
            patient.birthdate,
            patient.weight,
            patient.gender
          );
      ...
      // Set dataProcessed flag to true after processing
  ...
  return (
    <div style={{ overflow: "auto" }}>
      ...
      >
        <Box
          sx={{ display: "flex", alignItems: "center", marginBottom: "1px" }}
        >
          <Typography
            variant="body1"
            component="div"
        ...
        <div>
          <button
            onClick={() =>
              downloadAsPdf("reportTable", chosenBarangay + "_summaryReport")
            }
          >
            Download as PDF
          </button>
